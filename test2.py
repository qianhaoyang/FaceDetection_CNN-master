...
caffe_root = '/home/anybody/caffe-master/'
...
def face_detection(imgList):
  ...
  for imgFile in open(imgList).readlines():
    ...
    scales = []
    factor = 0.7937
    img = Image.open("test/1/" + imgFile.strip())
    print "-----------------------------"
    print "Image:"+imgFile.strip()+" count:"+str(img_count)
    min = 0
    max = 0
    ...
    factor_count = 1
    while(min >= 227):
	if(max * pow(factor, factor_count)<= 1800 ):
	...
	print "scale:"+str(scale)+" size:"+str(scale_img.size[1])+"*"+str(scale_img.size[0])
	scale_img.save("tmp.jpg")
        im = caffe.io.load_image("tmp.jpg")

        #reshape the data blob and make transformer
    ...
    true_boxes = nms_average(np.array(true_boxes1), overlapThresh=0.1)
#    true_true_boxes = nms_average(np.array(true_boxes), overlapThresh=0.2)
#   true_boxes_average = nms_average(boxes_nms,overlapThresh=0.2)
   ...
   # for box in total_boxes:
    ...
    img.save("result/1/" + str(img_count) + ".jpg")
    img_count += 1
    #end = time.clock()
  ...
  all_end = time.clock()
  print "Finish:"
  print "The run time is : %.03f seconds" %(all_end - all_start)
  print "All image numbers :",img_count

if __name__ == "__main__":
   #convert_full_conv()
   face_detection("list1.txt")
